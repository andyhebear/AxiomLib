...
namespace Axiom.SceneManagers.PagingLandscape.Renderable
{
	...
	public class IndexBuffer: IDisposable
	{
		...
		protected List<Dictionary<int, IndexData>> levelIndex;
		...
		public IndexBuffer() 
		{
			...
			cache = new ArrayList();
			levelIndex = new List<Dictionary<int, IndexData>>();//= new ArrayList((int)numIndexes);
			for ( long i = 0; i < numIndexes; i++ )
			{
				levelIndex.Add( new Dictionary<int,IndexData>() );
			}
		}
		...
		*/
		...
		{
			Dictionary<int, ArrayList> ll;

			
			Debug.Assert (levelIndex[ (int)RenderLevel ] != null);
			IndexData ii = null;
			levelIndex[(int)RenderLevel].TryGetValue( (int)stitchFlags, out ii );
		//	IEnumerator ii = ((Axiom.Collections.Map)(levelIndex[ (int)RenderLevel ])).Find( (long)stitchFlags );
			if ( ii == null)
			{
				...
				//( (IDictionary)( levelIndex[ (int)RenderLevel ] ) ).Add( (long)stitchFlags, indexData );
				levelIndex[(int)RenderLevel].Add( (int)stitchFlags, indexData );
				return indexData;
			}
			else
			{
