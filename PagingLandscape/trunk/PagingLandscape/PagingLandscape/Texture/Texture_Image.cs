		...
		protected override void loadMaterial()
		{
			if ( material == null )
			{
				...
				if ( Options.Instance.ImageNameLoad )
				{
					...
					material.GetTechnique( 0 ).GetPass( 0 ).GetTextureUnitState( 0 ).SetTextureAddressingMode( TextureAddressing.Clamp );//TextureAddressingMode = new  = TextureAddressing.Clamp;
				}
				else
				{
					// JEFF - all material settings configured through material script
