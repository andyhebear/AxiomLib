...
namespace Axiom.SceneManagers.PagingLandscape.Page
{
	...
	*/
		...
		*/
		...
		/// <summary>
		/// Test tile <c>sample</c>
		///<param name="myParm">Desc</param> 
		///<remarks>Determines if <paramref name="myParm"/> is either <c>null</c> or <c>empty</c>.</remarks>
		/// </summary>
		/// <param name="i"></param>
		/// <param name="j"></param>
		/// <returns></returns>
		...
		*/
		...
		{
			if ( Utility.Intersects(box, boundsExt ) != Intersection.None )
			{
				for ( long i = 0; i < numTiles; i++ )
				{
					for ( long j = 0; j < numTiles; j++ )
