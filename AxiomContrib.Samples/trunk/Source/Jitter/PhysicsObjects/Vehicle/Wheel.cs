...
namespace AxiomContrib.Samples.Jitter.PhysicsObjects.Vehicle
{
    ...
    public class Wheel
    {
        ...
        public Wheel(World world,RigidBody car,JVector position,float radius)
        {
            ...
            world.PreStep += new WorldStep(PreStep);
            world.PostStep += new WorldStep(PostStep);
        ...
        private void PreStep(float timeStep)
        {
            ...
            for (int i = 0; i < NumberOfRays; i++)
            {
                ...
                float zOffset = Radius * (1.0f - (float)System.Math.Cos(System.Math.PI / 4 * (distFwd / Radius)));
            ...
            displacementForceMag *= System.Math.Abs(JVector.Dot(groundNormal, worldAxis));
